...
class User(BaseModel):
    ...
    def has_role(self, role_name: str) -> bool:
        ...
        """
        return any(
            role.name == role_name and role.is_active for role in self.roles
        )
...
class AuditLog(BaseModel):
    ...
    )  # Model name or resource type
    resource_id = Column(UUID(as_uuid=True), nullable=True)
    resource_repr = Column(
    ...
    # Relationships
    user = relationship(
        'User', foreign_keys=[user_id], back_populates='audit_logs_created'
    )
