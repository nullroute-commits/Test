...
class RBACManager:
    ...
    def get_user_permissions(self, user_id: str, use_cache: bool = True) -> Set[str]:
        ...
        with get_db_session() as session:
            ...
            if user.is_superuser:
                all_permissions = (
                    session.query(Permission).filter(Permission.is_active).all()
                )
    ...
    def assign_role(
        self, user_id: str, role_name: str, assigned_by: Optional[str] = None
    ) -> bool:
        ...
        try:
            with get_db_session() as session:
                ...
                role = session.query(Role).filter(
                    Role.name == role_name, Role.is_active
                ).first()
    ...
    def revoke_role(
        self, user_id: str, role_name: str, revoked_by: Optional[str] = None
    ) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Failed to revoke role {role_name} from user {user_id}: {str(e)}"
            )
            return False
